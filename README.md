...
KengaCAD_Professional_X.X.X_android.apk — Android (упрощённый jog + экспорт KRL/RAPID/G-code, не полный CAD).

Новые версии появляются, когда на репозитории ставят тег вида v2.2.0. GitHub Actions сам собирает все варианты и прикрепляет к Release.
...
Нужны: .NET 8 SDK (desktop), .NET 9 SDK + MAUI workload (Android), Windows, для установщика — Inno Setup 6.
...
Android APK (из корня, нужен JDK 17 и `dotnet workload install android maui`):
...
1. Подними версию в KengaCAD/KengaCAD.csproj, KengaCAD.Mobile/KengaCAD.Mobile.csproj и installers/KengaCAD_Professional.iss.
...
Workflow .github/workflows/release.yml соберёт Windows (Setup + ZIP), Linux portable (+ AppImage если получится), macOS portable, Android APK и создаст Release с файлами.
...
KengaCAD/ — исходники C# WPF (основной продукт, Windows)
KengaCAD.Core/ — общая логика роботов, постпроцессоры (Scriban 7.x)
KengaCAD.Mobile/ — MAUI-клиент для Android
